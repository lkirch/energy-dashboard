water_compare_today_yesterday_usage <- function(dataid,date) {
  #date <- '2014-07-13'
  dt <- ymd(date)
  ...
  query <- paste(
    ...
    and month = '",month,"' 
    and day = '",date,"' 
    and cast(hour as int) between 0 and ",hour,"
    ) / 
    (
    select sum(usage) 
    from water_per_hour 
    ...
    and month = '",ymonth,"' 
    and day = '",ydate,"' 
    and cast(hour as int) between 0 and ",hour,") - 1) as pct_diff
    ",sep = "")
  query <- gsub("\n"," ",query)
  query <- gsub("\t"," ",query)
  #query
...
#water_compare_today_yesterday_usage('1185','2014-07-15')
