get_elec_prev_month <- function(dataid, date) {
...
#  library(stringr)
  ...
  # need to go back 2 months for the data since we are looking for the previous month
  end_date <- getStartOfTimeFrame(as.Date(date, format='%Y-%m-%d'),'month')
